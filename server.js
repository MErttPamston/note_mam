...
function defaultData() {
  return {
    codes: [
      { code: '6551', role: 'admin', name: 'Мама' }
    ],
    groups: [
      { id: 'g_lepka', name: 'Лепка', price: 2200, sickPrice: 1800, duration: 60 }
    ],
    students: [],
...
function computeAutoDay(dateStr) {
  ...
  for (const s of data.students) {
    if (s.recur && dateMatchesRecur(dateStr, s.recur)) {
      ...
      byGroup.get(s.groupId).push({ studentId: s.id, status: 'planned' });
    }
  }
  const sessions = [];
  for (const [groupId, students] of byGroup.entries()) {
...
function priceFor(groupId, status, override) {
  ...
  const group = data.groups.find(g => g.id === groupId);
  if (!group) return 0;
  if (status === 'sick') return group.sickPrice != null ? group.sickPrice : group.price;
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (p === '/api/groups' && req.method === 'POST') {
      ...
      const g = { id: uid('g'), name: (b.name || 'Занятие').trim(), price: Number(b.price) || 0, sickPrice: Number(b.sickPrice) || 0, duration: Number(b.duration) || 60 };
    ...
    if (p.startsWith('/api/groups/') && req.method === 'PUT') {
      ...
      if (b.price != null) g.price = Number(b.price) || 0;
      if (b.sickPrice != null) g.sickPrice = Number(b.sickPrice) || 0;
    ...
    if (p === '/api/students' && req.method === 'POST') {
      const auth = requireAdmin(req, res); if (!auth) return;
      const b = JSON.parse(await readBody(req) || '{}');
      const s = { id: uid('s'), name: (b.name || '').trim(), phone: b.phone || '', groupId: b.groupId || null, recur: b.recur || null, note: b.note || '' };
    ...
    if (p.startsWith('/api/students/') && req.method === 'PUT') {
      ...
      const b = JSON.parse(await readBody(req) || '{}');
      if (b.name != null) s.name = b.name.trim();
      if (b.phone != null) s.phone = b.phone;
      if (b.groupId !== undefined) s.groupId = b.groupId;
      if (b.recur !== undefined) s.recur = b.recur;
      if (b.note != null) s.note = b.note;
      saveData();
      return sendJson(res, 200, { ok: true, student: s });
    }
    ...
    if (p.startsWith('/api/day/') && req.method === 'GET') {
      ...
      const enriched = day.sessions.map(sess => ({
        ...
        students: sess.students.map(st => {
          ...
          return { ...st, name: student ? student.name : '(удалён)', price: priceFor(sess.groupId, st.status, st.priceOverride) };
        })
      }));
